...
// input_my
let amounel = document.getElementById('amount');
let amount = amounel.value;


let plus = () =>{
  amount++;
  amounel.value = amount;
...
//hien thị gioi thieu , bao quan

const gtmenu = document.querySelectorAll(".gt");
const subgt = document.querySelectorAll('.sub-gt');
const gti = document.querySelectorAll('#gti');

gtmenu.forEach((item,index) =>{
  item.addEventListener('click', ()=>{
    subgt[index].classList.toggle('active');
  })
})